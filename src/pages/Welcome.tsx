...
const styles = `
  ...
  .orb1 { width:380px; height:380px; background:#a50044; top:-80px; left:-80px; }
  .orb2 { width:300px; height:300px; background:#004d98; bottom:-60px; right:-60px; animation-delay:-5s; }
  .orb3 { width:200px; height:200px; background:#edbb00; bottom:25%; right:10%; animation-delay:-3s; opacity:0.08; }
  ...
  .score-status.live { background:rgba(219,0,48,0.3); color:#ff6b6b; animation:pulse 1.5s infinite; }
  .score-status.upcoming { background:rgba(0,77,152,0.3); color:#60a5fa; }
  @keyframes pulse { 0%,100%{opacity:1;} 50%{opacity:0.5;} }
  .live-dot { display:inline-block; width:6px; height:6px; background:#ff6b6b; border-radius:50%; margin-right:4px; animation:pulse 1s infinite; }
  ...
  .score-time { font-size:9px; color:rgba(255,255,255,0.35); letter-spacing:0.06em; font-weight:600; }
    ...
    const fetchMatch = async () => {
      try {
        const res = await fetch(
          "https://free-api-live-football-data.p.rapidapi.com/football-get-all-fixtures-by-team-id?teamid=1161&seasonid=23927",
          {
            headers: {
        ...
        if (data && data.response && data.response.length > 0) {
          ...
          const live = fixtures.find((f: any) => f.fixture?.status?.short === "1H" || f.fixture?.status?.short === "2H" || f.fixture?.status?.short === "HT");
          if (live) {
            const isHome = live.teams?.home?.id === 1161;
            setMatch({
              homeTeam: live.teams?.home?.name || "FC Barcelona",
              awayTeam: live.teams?.away?.name || "Opponent",
          ...
          if (finished) {
            const d = new Date(finished.fixture.date);
            setMatch({
              homeTeam: finished.teams?.home?.name || "FC Barcelona",
              awayTeam: finished.teams?.away?.name || "Opponent",
              homeScore: finished.goals?.home,
              awayScore: finished.goals?.away,
              status: "finished",
  ...
  return (
    <div className="scorebar">
      ...
      <div className="score-mid">
        ...
        {match.status === "upcoming" && <span className="score-status upcoming">Próximo</span>}
        <span className="score-vs">vs</span>
        {match.date && <span className="score-date">{match.date}</span>}
      </div>
      <div className="score-team opp">
        <span className="team-name">{oppName.length > 10 ? oppName.substring(0, 10) + "..." : oppName}</span>
